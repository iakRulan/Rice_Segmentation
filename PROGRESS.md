...
## ✅ 最终结果（2026-08-06，本地全流程自主完成）
**提交包就绪：`submission_final_v2.zip`（blend_final，阈值扫描后）**

| 评估口径 | wheat | rape | rice | MEAN |
|----------|-------|------|------|------|
| 全 val（final settings） | 0.9280 | 0.8875 | 0.8974 | **0.9043** |
| valhold（公平集，s7/tp 未见） | 0.9047 | 0.9153 | 0.8701 | **0.8967** |
| **诚实 holdout**（一半拟合/一半评估） | 0.9284 | 0.8866 | 0.8974 | **0.9041** |

**线上 0.8674 → 预估 ~0.91+**（诚实 holdout 0.9041 已破 90 目标）。

**最终模型**（全部 trainplus 训练，deeplabv3plus/mit_b3 + focal）：
- **tp_wr**（小麦/油菜多类）：valhold 0.898 —— 小麦 blend 权重 0.905
- **tp_r**（水稻）：valhold 0.872 —— 水稻 blend 权重 0.982（水稻 0.8475→0.897）
- s7（伪标签+focal）、v4、seed43 专用模型做辅助
- 提交设置：wheat t=0.55 pp=(30,30)；rape t=0.55 pp=(0,60)；rice t=0.41 pp=(0,0)
- 空图分类器：小麦/油菜 GBT、水稻 logistic
