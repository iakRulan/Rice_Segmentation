...
def train_one_epoch(model, dataloader, criterion, optimizer, scaler, device, epoch, log_interval=50):
    ...
    for batch_idx, (images, masks, _) in enumerate(dataloader):
        ...
        with autocast():
            ...
            # Handle shape mismatch for single-label mode
        ...
        running_iou += compute_batch_iou(outputs, masks)
        num_batches += 1
        
        if (batch_idx + 1) % log_interval == 0:
            print(f"  Epoch {epoch} [{batch_idx+1}/{len(dataloader)}] Loss: {running_loss/num_batches:.4f} IoU: {running_iou/num_batches:.4f}")
...
def main():
    parser = argparse.ArgumentParser(description='Train model with extended training')
    ...
    if args.mode == 'wheat_rape':
        train_image_dir = os.path.join(data_dir, 'train/image/wheat_rape')
        train_label_dirs = [
        ...
        train_image_dir = os.path.join(data_dir, 'train/image/rice')
        train_label_dirs = [os.path.join(data_dir, 'train/label/rice')]
        val_image_dir = os.path.join(data_dir, 'val/image/rice')
        val_label_dirs = [os.path.join(data_dir, 'val/label/rice')]
    
    # 数据增强
    train_transform = get_training_augmentation(256)
    val_transform = get_validation_augmentation(256)
    ...
    train_loader = DataLoader(train_dataset, batch_size=args.batch_size, shuffle=True, num_workers=8, pin_memory=True, drop_last=True, persistent_workers=True)
    ...
    # 保存训练历史
    history_path = os.path.join(args.output_dir, f'history_{args.mode}.json')
