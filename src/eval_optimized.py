...
def apply_tta(model, image_dir, device, img_size=256):
    ...
    for img_name in tqdm(images, desc='TTA'):
...
def main():
    ...
    wheat_rape_results = apply_tta(wheat_rape_model, os.path.join(val_image_dir, 'wheat_rape'), device)
    rice_results = apply_tta(rice_model, os.path.join(val_image_dir, 'rice'), device)
    ...
    # Now try with different thresholds
    print("\n" + "="*60)
    ...
    for wheat_thresh in np.arange(0.3, 0.7, 0.05):
        for rape_thresh in np.arange(0.3, 0.7, 0.05):
            for rice_thresh in np.arange(0.3, 0.7, 0.05):
                # Compute IoU with these thresholds
                w_ious = []
                for img_name in sorted(wheat_rape_results.keys()):
                    pred = wheat_rape_results[img_name][0]
